...
def welcome():
    ...
    return render_template("welcome.html",images=image_files)
...
def get_all_gestures():
    """
    Fetch all gestures and serialize ObjectIds properly.
    """
    logger.debug("Received request to fetch all gestures.")

    try:
        # Fetch gestures
        raw_gestures = dictionary_collection.find(
            {},
            {"_id": 0, "contributor_name": 1, "name": 1, "category": 1,
             "created_at": 1, "avatar_video_id": 1}
        )

        gestures = []
        for gesture in raw_gestures:
            # If avatar_video_id exists, convert it to string
            if 'avatar_video_id' in gesture and gesture['avatar_video_id']:
                gesture['avatar_video_id'] = str(gesture['avatar_video_id'])

            gestures.append(gesture)

        logger.debug(f"Fetched {len(gestures)} gestures successfully.")
...
def serve_avatar_video(video_id):
    """
    Streams the video file associated with a given avatar_video_id.
    """
    try:
        ...
        return Response(
            video_file.read(),
            mimetype='video/mp4'  # IMPORTANT: Tell browser it's a video!
        )
    
    except Exception as e:
        logger.error(f"Error streaming avatar video {video_id}: {str(e)}")
...
def get_gestures_by_category(category):
    try:
        gestures = list(dictionary_collection.find(
            {"category": category},
            {"_id": 0, "contributor_name": 1, "name": 1, "category": 1, 
             "created_at": 1, "avatar_video_url": 1}   # Added thumbnail_url
        ))
        return jsonify({'success': True, 'gestures': gestures})
    except Exception as e:
...
# Frontend Routes
@app.route('/lectures')
@app.route('/lectures/<playlist_id>')
def lecture_gestures(playlist_id=None):
    """Frontend page for browsing gestures with optional playlist ID"""
    return render_template('lectures.html', playlist_id=playlist_id)
...
@app.route('/gallery')
def gallery():
    # Get the list of image filenames in the static/images folder
    image_files = [f for f in os.listdir(IMAGE_FOLDER) if f.endswith('.webp')]
...
@app.route('/gesture')
def gesture_details():
    """Route to show gesture details page with recommendations"""
    gesture_name = request.args.get('name')
    if not gesture_name:
        return render_template('gesture_details.html', error="No gesture specified")
    ...
    try:
        ...
        # Try to get recommended gestures from same category
        recommended = list(dictionary_collection.find(
            {
                "category": gesture.get("category", ""),
                "name": {"$ne": gesture["name"]}  # Exclude current gesture
            },
            {"_id": 0, "name": 1, "category": 1, "avatar_video_id": 1, "image_url": 1}
        ).limit(6))
        
        # If not enough in same category, get alphabetically sorted gestures
        if len(recommended) < 4:
            all_gestures = list(dictionary_collection.find(
                {"name": {"$ne": gesture["name"]}},
                {"_id": 0, "name": 1, "category": 1, "avatar_video_id": 1, "image_url": 1}
            ).sort("name", 1).limit(6))
            
            # Combine and deduplicate
            combined = recommended + [g for g in all_gestures if g not in recommended]
            recommended = combined[:6]
        
        # Add video URLs to recommended gestures
        for g in recommended:
            if "avatar_video_id" in g:
                g["video_url"] = url_for('stream_video', 
                                        video_id=g["avatar_video_id"], 
                                        _external=True)
        
        return render_template('gesture_details.html', 
                             gesture=gesture,
                             recommended_gestures=recommended)
...
def admin_login():
    if request.method == 'POST':
        ...
        if admin and check_password_hash(admin['password'], password):
            ...
            flash('Login successful!', 'success')
            return redirect(url_for('admin_dashboard'))  # Define this route later
        else:
...
def admin_dashboard():
    # User role counts
    contributors_count = db["users"].count_documents({})
    sigml_generators_count = db["sigml_generators"].count_documents({})
    editors_count = db["editors"].count_documents({})

    # Gesture counts
    pending_count = db["pending_gestures"].count_documents({})
    approved_count = db["approved_gestures"].count_documents({})
    revision_count = db["revision_gestures"].count_documents({})
    rejected_count = db["rejected_gestures"].count_documents({})
    total_gestures = pending_count + approved_count + revision_count + rejected_count

    # Final gesture location count
    final_location_count = db["final_gesture_collection"].count_documents({})

    # Count gestures added to finalgesturecollection in the last month
    one_month_ago = datetime.now() - timedelta(days=30)
    last_month_final = db["final_gesture_collection"].count_documents({
        "timestamp": {"$gte": one_month_ago}
    })

    return render_template("admin_dashboard.html",
                           contributors=contributors_count,
                           sigml=sigml_generators_count,
                           editors=editors_count,
                           total_gestures=total_gestures,
                           final_location_count=final_location_count,
                           last_month_final=last_month_final)
...
def create_user():
    if request.method == 'POST':
        ...
        if (editors_collection.find_one({'email': email}) or 
            sigml_generators_collection.find_one({'email': email})):
        ...
        try:
            if role == 'editor':
                ...
                editors_collection.insert_one(editor_data)
                flash('Editor created successfully', 'success')

            elif role == 'sigml_generator':
                # Create SIGML Generator
                generator_data = {
                    'username': username,
                    'email': email,
                    'password': hashed_password,
                ...
                sigml_generators_collection.insert_one(generator_data)
                flash('SIGML Generator created successfully', 'success')
...
def view_users():
    # Fetch all editors
    editors = list(editors_collection.find({}, {
        'username': 1,
        'email': 1,
        'created_at': 1,
        '_id': 0
    ...
    # Fetch all SIGML generators
    sigml_generators = list(sigml_generators_collection.find({}, {
        'username': 1,
        'email': 1,
        'created_at': 1,
        '_id': 0
    ...
    for editor in editors:
        ...
        if 'created_at' not in editor:
            editor['created_at'] = datetime.now()
    
    for generator in sigml_generators:
        generator['role'] = 'SIGML Generator'
        # Add default created_at if missing
        if 'created_at' not in generator:
            generator['created_at'] = datetime.now()
    
    # Combine both lists
    all_users = editors + sigml_generators
    
    # Sort by creation date, handling cases where created_at might be None
